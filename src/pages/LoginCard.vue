  ...
  axios
    ...
    .catch((error) => {
      ...
      if (err.value) {
        ...
        err.value = 'ลอคอินไม่ได้เนื่องจากรออัพเดทตามระบบใหม่ของมหาลัย'
      }
    })
    .finally(() => {
      loading.value = false
      ...
      <form class="p-5 max-w-sm flex flex-col dark:bg-gray-800" @submit.prevent="login">
        <div class="md:flex-grow">
          ...
          <h3 class="text-sm text-gray-600 dark:text-white">
            **ตารางเรียนจะอัพเดทตามตารางล่าสุดในเว็บมหาลัย <span class="text-red-400">รอแก้ตามระบบมหาลัยใหม่</span>
          </h3>
          <input
            v-model="username"
            type="text"
